...
class DeliveryAvailableOrdersView extends StatefulWidget {
  ...
  @override
  State<DeliveryAvailableOrdersView> createState() =>
      _DeliveryAvailableOrdersViewState();
}

class _DeliveryAvailableOrdersViewState
    extends State<DeliveryAvailableOrdersView> {
  ...
  Future<void> _acceptOrder(int idPedido) async {
    ...
    final success =
        await dbService.asignarPedido(idPedido, widget.deliveryUser.idUsuario);

    bool estadoActualizado = false;
    if (success) {
      // Cambiar estado a 'en camino' tras asignar
      estadoActualizado =
          await dbService.updatePedidoEstado(idPedido, 'en camino');
    }
    ...
    if (mounted) {
      messenger.showSnackBar(SnackBar(
        content: Text(success
            ? (estadoActualizado
                ? 'Pedido aceptado y en camino. Revisa "Mis Entregas"'
                : 'Pedido aceptado, pero no se pudo actualizar el estado.')
            : 'El pedido ya fue tomado.'),
        backgroundColor:
            success && estadoActualizado ? Colors.green : Colors.orange,
      ));
      if (success && estadoActualizado) {
        _loadPedidos();
      }
    }
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Debes iniciar sesión para ver pedidos disponibles',
        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return _OrderCard(
                  pedido: pedido,
                  onAccept: () => _acceptOrder(pedido.idPedido));
            },
          ),
        );
      },
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text('No hay pedidos disponibles por ahora',
              style: TextStyle(fontSize: 18, color: Colors.grey)),
          const SizedBox(height: 16),
          ElevatedButton.icon(
...
class _OrderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final formattedDate =
        DateFormat('dd MMM, hh:mm a').format(pedido.fechaPedido);
    return Card(
      elevation: 3,
          ...
          children: [
            ...
            SizedBox(
                ...
                child: ElevatedButton(
                    onPressed: onAccept, child: const Text('Aceptar Pedido'))),
          ],
        ),
      ),
    );
