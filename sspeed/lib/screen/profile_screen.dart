...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Negocio? _negocioAsociado;
  bool _cargandoNegocio = false;
  ...
  void initState() {
    ...
    _verificarNegocio();
    // Permisos de ubicacion ya no son necesarios aqui (mapa movido a Home)
  }

  Future<void> _verificarNegocio() async {
    setState(() => _cargandoNegocio = true);
    try {
      final db = Provider.of<DatabaseService>(context, listen: false);
      final negocio = await db.getNegocioDeUsuario(widget.usuario.idUsuario);
      if (!mounted) return;
      setState(() => _negocioAsociado = negocio);
    } catch (e) {
      // Silenciar error, no bloquea la UI
    } finally {
      if (mounted) setState(() => _cargandoNegocio = false);
    }
  }

  Future<void> _openSite() async {
    final uri = Uri.tryParse(AppConfig.siteUrl);
    if (uri == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('URL del sitio no valida'),
  ...
  Future<void> _copySiteUrl() async {
    await Clipboard.setData(ClipboardData(text: AppConfig.siteUrl));
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
  ...
  // Permisos de ubicacion removidos del perfil
  ...
  // Mapa de ubicaciones removido de Perfil (ahora se muestra en Home)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor:
          const Color(0xFFF5F5F5), // Fondo gris claro para resaltar contenido
      appBar: AppBar(
      ...
      body: AdaptiveScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Column(
                children: [
                  ...
                  _buildMenuOption(
                    ...
                    color: Colors.green,
                    title: 'Opiniones',
                    subtitle: 'Comparte y revisa tus opiniones',
                    onTap: () => Navigator.of(context).pushNamed(
                      AppRoutes.opiniones,
                      arguments: widget.usuario,
                    ),
                  ),
                  // Solo muestra el botón si el usuario es cliente y no tiene negocio asociado
                  if (widget.usuario.rol == 'cliente' &&
                      _negocioAsociado == null) ...[
                    const Divider(height: 1, indent: 16, endIndent: 16),
                    _buildMenuOption(
                      context,
                      icon: Icons.store,
                      color: const Color(0xFFF97316),
                      title: _cargandoNegocio
                          ? 'Verificando negocio...'
                          : 'Convertirse en Negocio',
                      subtitle: _cargandoNegocio
                          ? 'Por favor espera'
                          : 'Registra tu negocio y vende productos',
                      onTap: _cargandoNegocio
                          ? () {}
                          : () async {
                              ...
                              await _verificarNegocio();
                            },
                    ),
                  ] else if (_negocioAsociado != null &&
                      widget.usuario.rol == 'cliente') ...[
                    const Divider(height: 1, indent: 16, endIndent: 16),
                    _buildMenuOption(
                      context,
                      ...
                      subtitle:
                          'Hemos recibido tu registro. Activación pendiente.',
                      onTap: () {},
                    ),
                  ],
                  const Divider(height: 1, indent: 16, endIndent: 16),
            ...
            // MEJORA: Se a�ade un men� exclusivo para negocios
            if (widget.usuario.rol == 'negocio') ...[
              const SizedBox(height: 24),
              Text('Herramientas de Negocio',
                  style: theme.textTheme.titleMedium
                      ?.copyWith(color: Colors.grey[600])),
              const SizedBox(height: 8),
              Card(
                ...
                child: Column(
                  children: [
                    _buildMenuOption(
                      ...
                      subtitle: 'Dashboard de ventas y productos',
                      onTap: () {
                ...
                child: Column(
                  children: [
                    ...
                    _buildMenuOption(
                      ...
                      subtitle: 'Ver buenas, regulares y malas',
                      onTap: () {
                        Navigator.of(context).pushNamed(
                          AppRoutes.adminOpiniones,
                          arguments: widget.usuario,
                        );
                      },
                    ),
                    ...
                    _buildMenuOption(
                      ...
                      subtitle:
                          'Vista r�pida de clientes (verde) y repartidores (rojo)',
                      onTap: () {
                        Navigator.of(context).pushNamed(
            ...
            FutureBuilder<List<Ubicacion>>(
              ...
              builder: (context, snapshot) {
                ...
                // Mapa desactivado en Perfil (se movio a la pantalla principal)
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle:
                  const Text('Asistente autom�tico con respuestas r�pidas'),
                ...
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => ChatScreen(
                      initialSection: ChatSection.unibot,
            ...
            ListTile(
              ...
              subtitle: const Text('Contacta con nuestro equipo de ayuda'),
              trailing: const Icon(Icons.chevron_right),
              onTap: () {
                Navigator.pop(context);
                Navigator.of(context).push(
