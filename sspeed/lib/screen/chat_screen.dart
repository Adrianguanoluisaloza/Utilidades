...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _loadMessages() async {
    ...
    try {
      ...
      setState(() {
        ...
        _error = null;
        _currentPage = 0;
        _hasMoreMessages = initialMessages.length >= _messagesPerPage;
      });
      _scrollToBottom();
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadMoreMessages() async {
    ...
    try {
      ...
      final offset = (_currentPage + 1) * _messagesPerPage;

      final olderMessages = await dbService.getMensajesDeConversacion(
        _idConversacion!,
      ...
      // Convertir mensajes a ChatEntry
      ...
      setState(() {
        // Agregar mensajes antiguos al inicio de la lista
        _messages = [...entries, ..._messages];
        _currentPage++;
        _isLoadingMore = false;
        _hasMoreMessages = entries.length >= _messagesPerPage;
      });

      // Restaurar posición del scroll después de agregar mensajes
      ...
      debugPrint('[ChatScreen] Error cargando más mensajes: $e');
    }
  }
  ...
  Future<List<ChatEntry>> _loadInitialMessages() async {
    ...
    if (widget.initialSection == ChatSection.delivery ||
        widget.initialSection == ChatSection.unibot ||
        widget.initialSection == ChatSection.soporte) {
      String senderName;
      switch (widget.initialSection) {
        case ChatSection.delivery:
          senderName =
              widget.currentUser.rol == 'delivery' ? 'Cliente' : 'Repartidor';
          break;
  ...
  Future<Map<String, dynamic>> _sendToBackend(
    ...
    final response = await dbService.enviarMensaje(
      ...
      esBot: widget.initialSection == ChatSection.unibot,
    );
  ...
  Future<List<ChatEntry>> _getHistorial(int? idConversacion) async {
    ...
    // CORRECCIÓN: Se utiliza el DatabaseService centralizado con paginación.
    final dbService = context.read<DatabaseService>();
    final messageModels = await dbService.getMensajesDeConversacion(
      idConversacion,
      limit: _messagesPerPage,
      offset: 0,
    );
  ...
  String _getAppBarTitle() {
    switch (widget.initialSection) {
      ...
      case ChatSection.delivery:
        return 'Chat con tu Repartidor';
      case ChatSection.soporte:
        return 'Chat con Soporte';
      case ChatSection.unibot:
        return 'Asistente Virtual (Unibot)';
      case ChatSection.historial:
  ...
  String _getInitialBotMessage() {
    switch (widget.initialSection) {
      ...
      case ChatSection.delivery:
        return 'Hola! Soy tu repartidor. Estoy en camino con tu pedido. Si tienes alguna duda, escríbeme.';
      case ChatSection.soporte:
        return 'Bienvenido al chat de soporte. Por favor, describe tu problema.';
      case ChatSection.unibot:
        return 'Hola! Soy Unibot, tu asistente virtual. ¿Tienes alguna pregunta sobre nuestros productos o servicios?';
      case ChatSection.historial:
                ...
                children: [
                  Row(
                    children: [
                      ...
                      if (widget.initialSection == ChatSection.unibot) ...[
      ...
      body: Container(
        ...
        child: Column(
          children: [
            ...
            if (_isSending && widget.initialSection == ChatSection.unibot)
              Container(
                margin:
                    const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
