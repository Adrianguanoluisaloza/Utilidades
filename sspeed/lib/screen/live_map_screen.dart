...
class _LiveMapScreenState extends State<LiveMapScreen> {
  ...
  // Estilo ligero para reducir carga visual en Google Maps (oculta POIs, tránsito y límites administrativos)
  ...
  Timer? _mediumRefreshTimer;

  void _startAutoRefreshTimer() {
    // Timers desactivados. Solo se activan manualmente si el usuario lo requiere.
  ...
  // Evita repintados innecesarios del mapa cuando los marcadores no han cambiado
  bool _sameMarkers(Set<Marker> other) {
    if (other.length != _markers.length) return false;
    for (final m in other) {
      final match = _markers.where((e) => e.markerId == m.markerId);
      if (match.isEmpty) return false;
      final current = match.first;
      if (current.position != m.position || current.icon != m.icon) {
        return false;
      }
    }
    return true;
  ...
  Future<void> _refreshMarkers() async {
    ...
    try {
      ...
      if (mounted) {
        // Actualiza el estado solo si realmente hubo cambios para evitar repintados costosos
        ...
        // No agenda reintentos automáticos si no hay repartidores visibles
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _maybeUploadDeliveryLocation(LatLng position) async {
    ...
    // ✅ OPTIMIZACIÓN: Se aumenta la frecuencia de subida a 20 segundos para reducir carga en el servidor
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Refrescar lo que viene',
        ...
        children: [
          GoogleMap(
            initialCameraPosition: CameraPosition(
              ...
              tilt: 45, // Activa vista 3D
            ),
            markers: _markers,
            myLocationEnabled: _permissionStatus == PermissionStatus.granted,
            myLocationButtonEnabled:
                _permissionStatus == PermissionStatus.granted,
            liteModeEnabled: false, // Mapa completo interactivo
            zoomControlsEnabled: false,
            compassEnabled: true, // Activa brújula para navegación 3D
            mapToolbarEnabled: true,
            tiltGesturesEnabled: true, // Permite gestos de inclinación
            rotateGesturesEnabled: true, // Permite rotación
            buildingsEnabled: true, // Activa edificios 3D
            indoorViewEnabled: true, // Activa vista interior
