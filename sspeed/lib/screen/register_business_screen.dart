...
class _RegisterBusinessScreenState extends State<RegisterBusinessScreen> {
  ...
  Future<void> _guardar() async {
    ...
    if (widget.usuario.idUsuario <= 0) {
      messenger.showSnackBar(
        const SnackBar(
          content:
              Text('Sesión inválida. Por favor, inicia sesión nuevamente.'),
    ...
    try {
      ...
      if (respuesta != null && respuesta['negocio'] != null) {
        ...
        setState(() => _negocioActual = negocioGuardado);

        // CRÍTICO: Actualizar el rol del usuario en la sesión después de registrar negocio
        // Si backend devolvió usuario actualizado, usarlo; si no, actualizar rol manualmente
        final usuarioFinal =
            usuarioActualizado ?? widget.usuario.copyWith(rol: 'negocio');
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: _isLoading
                              ...
                              : [
                                  BoxShadow(
                                    color:
                                        const Color(0xFF0078D4).withAlpha(102),
                                    blurRadius: 15,
