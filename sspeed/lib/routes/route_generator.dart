...
import '../screen/admin_map_lite_screen.dart';
import '../screen/edit_profile_screen.dart';
import '../screen/login_screen.dart';
import '../screen/reset_password_request_screen.dart';
import '../screen/reset_password_confirm_screen.dart';
...
// import '../screen/checkout_screen.dart'; // Archivo no existe
import '../screen/order_success_screen.dart';
import '../support/support_home_screen.dart';
// import '../screen/product_detail_screen.dart'; // Archivo no existe
// import '../models/producto.dart'; // No necesario aquí
// import '../screen/chat_home_screen.dart'; // Archivo no existe
// import '../screen/register_business_screen.dart'; // Archivo no existe
import '../screen/negocio_home_screen.dart';
import '../screen/opiniones/opiniones_screen.dart';
import '../admin/opiniones_admin_screen.dart';
...
class RouteGenerator {
  static Route<dynamic> generateRoute(RouteSettings settings) {
    switch (settings.name) {
      ...
      case AppRoutes.resetRequest:
        return _slideUp(settings, const ResetPasswordRequestScreen());
      case AppRoutes.resetConfirm:
        return _slideUp(settings, const ResetPasswordConfirmScreen());
      ...
      /* DESHABILITADO - CheckoutScreen no existe
      case AppRoutes.checkout:
        final args = settings.arguments as Map<String, dynamic>?;
        if (args != null &&
            args['usuario'] is Usuario &&
            args['ubicacion'] is Ubicacion) {
          return _slideUp(
              settings,
              CheckoutScreen(
                  usuario: args['usuario'], ubicacion: args['ubicacion']));
        }
        return _redirectToLogin(settings);
      */
      ...
      case AppRoutes.adminHome:
        ...
        return _redirectToLogin(settings);
      case AppRoutes.adminMapLite:
        final usuario = settings.arguments;
        if (usuario is Usuario && usuario.rol.trim().toLowerCase() == 'admin') {
          return _fade(settings, AdminMapLiteScreen(adminUser: usuario));
        }
        return _redirectToLogin(settings);
      ...
      case AppRoutes.orderDetail:
        ...
        return _redirectToLogin(settings);
      /* DESHABILITADO - ChatHomeScreen no existe
      ...
      */
      case AppRoutes.orderHistory:
        ...
        return _redirectToLogin(settings);
      /* DESHABILITADO - ProductDetailScreen no existe
      case AppRoutes.productDetail:
        final args = settings.arguments;
        if (args is Map<String, dynamic> &&
            args['producto'] is Producto &&
            args['usuario'] is Usuario) {
          return _slideUp(
              settings,
              ProductDetailScreen(
        ...
        return _redirectToLogin(settings);
      */
      /* DESHABILITADO - RegisterBusinessScreen no existe
      ...
      */
      case AppRoutes.negocioHome:
        ...
        return _redirectToLogin(settings);
      case AppRoutes.opiniones:
        final usuario = settings.arguments;
        if (usuario is Usuario) {
          return _slideUp(settings, OpinionesScreen(usuario: usuario));
        }
        return _redirectToLogin(settings);
      case AppRoutes.adminOpiniones:
        final usuario = settings.arguments;
        if (usuario is Usuario && usuario.rol.trim().toLowerCase() == 'admin') {
          return _fade(settings, OpinionesAdminScreen(adminUser: usuario));
        }
        return _redirectToLogin(settings);
      default:
        return _redirectToLogin(settings);
