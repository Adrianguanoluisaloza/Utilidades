...
public class NegocioController {
    ...
    public ApiResponse<Map<String, Object>> registrarONActualizar(int idUsuario, Negocio payload) {
        if (idUsuario <= 0) {
            throw new ApiException(400, "Identificador de usuario invalido");
        }
        ...
        try {
            ...
            if (existente.isPresent()) {
                ...
                resultado = negocioRepo.create(payload);
            }
            
            // CRÍTICO: Actualizar el rol del usuario a 'negocio'
            if (!"negocio".equalsIgnoreCase(usuario.getRol())) {
                usuarioRepo.updateRol(idUsuario, "negocio");
                usuario.setRol("negocio"); // Actualizar objeto en memoria para devolverlo
            }
            
            // Incluir usuario actualizado en la respuesta para sincronizar frontend
            Map<String, Object> respuesta = new HashMap<>();
            respuesta.put("negocio", resultado);
            respuesta.put("usuario", usuario);
            
            return ApiResponse.success(existente.isPresent() ? 200 : 201, 
                    existente.isPresent() ? "Negocio actualizado correctamente" : "Negocio registrado correctamente", 
                    respuesta);
        } catch (SQLException e) {
