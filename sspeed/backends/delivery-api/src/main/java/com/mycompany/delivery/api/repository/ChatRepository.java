...
public class ChatRepository {
    ...
    public Map<String, Object> guardarMensaje(com.mycompany.delivery.api.model.Mensaje mensaje) {
        ...
        long idPedido = mensaje.getIdPedido();
        long idCliente = mensaje.getIdRemitente(); // Asumimos que el remitente es el cliente
    ...
    public java.util.List<java.util.Map<String, Object>> obtenerChatPorPedido(long idPedido) {
        try {
     ...
     */
    public void ensureConversation(long idConversacion, Long idCliente, Long idDelivery, Long idAdminSoporte,
            Long idPedido, boolean esChatbot) throws SQLException {
        String sql = """
    ...
    public Map<String, Object> insertMensaje(long idConversacion, //
            long idRemitente, Long idDestinatario, String mensaje) throws SQLException {
        String sql = """
    ...
    public List<Map<String, Object>> listarMensajes(long idConversacion) throws SQLException {
        return listarMensajes(idConversacion, -1, 0);
    }

    /**
     * Lista los mensajes de una conversación con paginación.
     *
     * @param idConversacion el ID de la conversación.
     * @param limit el número máximo de mensajes a retornar (-1 para sin límite).
     * @param offset el offset desde el cual iniciar (para paginación).
     * @return una lista de mensajes (cada uno como un mapa con sus datos).
     * @throws SQLException Si ocurre un error en la base de datos.
     */
    public List<Map<String, Object>> listarMensajes(long idConversacion, int limit, int offset) throws SQLException {
        ...
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement(sql.toString())) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    row.put("id_remitente", rs.getLong("id_remitente"));
                    row.put("id_destinatario", rs.getObject("id_destinatario") != null ? rs.getLong("id_destinatario") : null);
     ...
     * @throws SQLException Si ocurre un error en la base de datos.
     */
    public List<Map<String, Object>> listarConversacionesPorUsuario(long idUsuario) throws SQLException {
        String sql = """
        ...
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) {
                    Map<String, Object> row = new HashMap<>();
                    row.put("id_conversacion", rs.getLong("id_conversacion"));
                    row.put("id_pedido", rs.getObject("id_pedido") != null ? rs.getLong("id_pedido") : null);
                    row.put("id_cliente", rs.getObject("id_cliente") != null ? rs.getLong("id_cliente") : null);
                    row.put("id_delivery", rs.getObject("id_delivery") != null ? rs.getLong("id_delivery") : null);
                    row.put("id_admin_soporte", rs.getObject("id_admin_soporte") != null ? rs.getLong("id_admin_soporte") : null);
     ...
     * @throws SQLException Si ocurre un error en la base de datos.
     */
    public long ensureConversationForUser(long idUsuario) throws SQLException {
        String sql = """
     ...
     * @throws SQLException Si ocurre un error en la base de datos.
     */
    public long ensureBotConversationForUser(long idUsuario) throws SQLException {
     ...
     */
    public long ensureBotUser() throws SQLException {
        // Seleccionar si ya existe el usuario BOT
        ...
        // Insertar el usuario BOT con rol 'soporte' utilizando id_rol del catálogo roles
        String insert = """
        ...
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement(insert)) {
            ...
            ps.setString(3, "chatbot123"); // Se hasheará por trigger si no está en formato bcrypt
            ps.executeUpdate();
        }

        // Reconsultar el ID creado
        try (Connection c = Database.getConnection(); PreparedStatement ps = c.prepareStatement(select)) {
            ps.setString(1, BOT_EMAIL);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
    ...
    private void ensureSchema() throws SQLException {
        final String createConversaciones = """
                CREATE TABLE IF NOT EXISTS chat_conversaciones (
                    ...
                    id_cliente BIGINT REFERENCES usuarios(id_usuario) ON DELETE SET NULL,
        ...
        final String createMensajes = """
                CREATE TABLE IF NOT EXISTS chat_mensajes (
                    ...
                    id_remitente BIGINT REFERENCES usuarios(id_usuario) ON DELETE SET NULL,
                    id_destinatario BIGINT REFERENCES usuarios(id_usuario) ON DELETE SET NULL,
