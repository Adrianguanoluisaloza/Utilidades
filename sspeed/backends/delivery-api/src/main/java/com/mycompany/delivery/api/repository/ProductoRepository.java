...
/**
 * Acceso a datos para productos. Todas las consultas respetan el esquema
 * actual: productos(id_negocio, id_categoria, ...).
 */
public class ProductoRepository {
    ...
    /**
     * Lista las categorías directamente desde la tabla 'categorias'. Incluye el
     * id y el nombre, ordenadas alfabéticamente. Esto permite mostrar
     * categorías aunque aún no tengan productos asociados.
     */
    public List<java.util.Map<String, Object>> listarCategoriasDb() throws SQLException {
        ...
        try (Connection conn = Database.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                ...
                cat.put("id", rs.getInt("id_categoria"));
