...
public final class SoporteRepository {
     ...
     */
    public long ensureSoporteConversacion(long idUsuario, String rol) throws SQLException {
    ...
    private Long buscarConversacionActiva(Connection conn, long idUsuario) throws SQLException {
        String sql = """
    ...
    public void insertMensajeUsuario(long idConversacion, long idRemitente, String mensaje) throws SQLException {
        String sql = """
    ...
    public void insertMensajeSoporte(long idConversacion, long idSoporte, String mensaje) throws SQLException {
        String sql = """
                INSERT INTO soporte_mensajes (id_soporte_conv, id_remitente, es_agente, tipo, mensaje, created_at)
    ...
    public void asignarHumano(long idConversacion, long idAgente) throws SQLException {
        String sql = """
    ...
    public long ensureBotSoporte() throws SQLException {
        ...
        final String insert = """
                ...
                VALUES (?, ?, crypt('soporte_bot', gen_salt('bf')),
                       (SELECT id_rol FROM roles WHERE LOWER(nombre) = 'soporte' LIMIT 1))
    ...
    // ============ MÉTODOS ALIAS PARA ENDPOINT DE SOPORTE ============
    
    /**
     * Alias para ensureSoporteConversacion con parámetros simplificados
     */
    public long obtenerOCrearConversacion(long idUsuario) throws SQLException {
        return ensureSoporteConversacion(idUsuario, "cliente");
    }

    /**
     * Método unificado para insertar mensajes (usuario o agente)
     */
    public void insertarMensaje(long idSoporteConv, Long idRemitente, boolean esAgente, String mensaje) throws SQLException {
        if (esAgente) {
            // Si es agente pero no tiene ID, usar bot de soporte
            long idSoporte = (idRemitente != null) ? idRemitente : ensureBotSoporte();
            insertMensajeSoporte(idSoporteConv, idSoporte, mensaje);
        } else {
            if (idRemitente == null || idRemitente <= 0) {
                throw new SQLException("idRemitente es obligatorio para mensajes de usuario");
            }
            insertMensajeUsuario(idSoporteConv, idRemitente, mensaje);
        }
    }
}
