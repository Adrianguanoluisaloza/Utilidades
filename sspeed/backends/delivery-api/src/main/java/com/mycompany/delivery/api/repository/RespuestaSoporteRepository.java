...
public final class RespuestaSoporteRepository {
    ...
    public int crearAutoRespuesta(SoporteRespuestaPayload payload) throws SQLException {
        ...
        try (Connection conn = Database.getConnection()) {
            ...
            String sql = """
                    INSERT INTO ia_respuestas_automaticas
                        (id_categoria_ia, canal, scope_destino, intent, keywords, regex_match,
                         respuesta, idioma, tono, prioridad, activo, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, COALESCE(?, TRUE), NOW(), NOW())
                    RETURNING id_respuesta_ia
                    """;
    ...
    public List<Map<String, Object>> listarAutoRespuestas(String categoria) throws SQLException {
        StringBuilder sql = new StringBuilder("""
                SELECT p.id_respuesta_ia,
                       ...
                       p.created_at,
                       p.updated_at
                FROM ia_respuestas_automaticas p
                LEFT JOIN ia_categorias_respuesta c ON p.id_categoria_ia = c.id_categoria_ia
                """);
        ...
        try (Connection conn = Database.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    map.put("id_respuesta", rs.getLong("id_respuesta_ia"));
    ...
    private Integer ensureCategoria(Connection conn, String categoria) throws SQLException {
        ...
        final String select = "SELECT id_categoria_ia FROM ia_categorias_respuesta WHERE LOWER(nombre) = LOWER(?)";
    ...
    // ============ MÉTODO SIMPLIFICADO PARA ENDPOINT DE SOPORTE ============
    
    /**
     * Busca una respuesta predefinida que coincida con el mensaje del usuario.
     * Usa la tabla soporte_respuestas_predef con patrón ILIKE separado por |
    ...
    public String buscarRespuesta(String mensaje) {
        ...
        String sql = """
                ...
                WHERE categoria = 'soporte'
                  AND activo = TRUE
                  AND (
                    pregunta_patron ILIKE '%' || ? || '%'
                    OR ? ~* ANY(string_to_array(pregunta_patron, '|'))
                  )
                ORDER BY prioridad ASC, id_predef DESC
                LIMIT 1
                """;
        ...
        try (Connection conn = Database.getConnection();
            ...
            ps.setString(1, mensajeLimpio);
            ps.setString(2, mensajeLimpio);
            ...
            System.err.println("⚠️ Error buscando respuesta predefinida: " + e.getMessage());
        }
    ...
    /**
     * Busca respuesta predefinida para el BOT (CIA/UniBot)
     * Similar al anterior pero para categoria='bot'
     */
    public String buscarRespuestaBot(String mensaje) {
        if (mensaje == null || mensaje.trim().isEmpty()) {
            return null;
        }
